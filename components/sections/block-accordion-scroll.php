...
$acc_items = [
  ...
  [
    ...
    'image' => '../assets/img/strengthen.png',
  ],
  [
...
<link rel="stylesheet" href="../assets/css/section-block_accordion_scroll.css" />
<link rel="stylesheet" href="../assets/css/components-accordion_scroll.css" />

<section class="block-accordion-scroll">
  <div class="block-accordion-scroll__wrapper">

    <!-- MEDIA / IMAGE -->
    <div class="block-accordion-scroll__media">
      <div class="accordion-card__image-wrapper">
        <img id="accordionImage" src="<?= $acc_items[0]['image'] ?>" alt="picture representing our impact" />
      </div>
    </div>

    <!-- ACCORDION -->
    <div class="block-accordion-scroll__description block-accordion-scroll__col--text">
      <div class="accordion-scroll" id="scrollContainer">
        <?php foreach ($acc_items as $index => $item): ?>
        <div class="item" id="item-<?= $index ?>" data-image="<?= $item['image'] ?>">
...
<script>
  ...
  function setActive(index) {
    ...
    allItems.forEach((item, i) => {
      ...
      if (i === index) {
        ...
        setTimeout(() => {
          const firstVisible = allItems[Math.max(index - 1, 0)];
          const firstHeight = firstVisible.offsetHeight;
          const scrollTop = item.offsetTop - firstHeight;
  ...
  container.addEventListener('wheel', throttle(onWheelStep, 1500), { passive: false });
