...
MultiSelect.prototype._render = function () {
  ...
  topLevel.forEach(function (opt) {
    var children = opt.termId && childrenMap[opt.termId] ? childrenMap[opt.termId] : [];
    if (children.length > 0) {
      // Parent with children: render as expandable group
      dropdownHtml += '<div class="dropdown-group" data-group="' + opt.termId + '">' + '<div class="dropdown-item dropdown-group__parent" data-checkbox="' + opt.id + '">' + '<div class="checkbox-wrapper">' + '<input type="checkbox" class="option-checkbox" id="' + opt.id + '" />' + '<span class="checkmark"></span>' + "</div>" + "<label>" + opt.name + "</label>" + '<span class="dropdown-group__arrow">&#8250;</span>' + "</div>" + '<div class="dropdown-group__sub">';
      children.forEach(function (child) {
        dropdownHtml += '<div class="dropdown-item" data-checkbox="' + child.id + '">' + '<div class="checkbox-wrapper">' + '<input type="checkbox" class="option-checkbox" id="' + child.id + '" />' + '<span class="checkmark"></span>' + "</div>" + "<label>" + child.name + "</label>" + "</div>";
      });
...
MultiSelect.prototype._syncInitialSelection = function () {
  ...
  this.optionEls.forEach(function (cb) {
    ...
    cb.checked = !!(targetOption !== null && targetOption !== void 0 && targetOption.selected);
  });
  if (this.selectAllEl) {
    this.selectAllEl.checked = this.optionEls.length > 0 && this.optionEls.every(function (c) {
      return c.checked;
    });
  }
  this.optionEls.forEach(function (cb) {
...
MultiSelect.prototype._updateParentState = function (changedCheckbox) {
  ...
  var checkedCount = childCheckboxes.filter(function (c) {
    return c.checked;
  }).length;
  if (checkedCount === 0) {
