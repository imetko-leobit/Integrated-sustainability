    ...
    if (navItem && panel.contains(navItem)) {
      ...
      if (isMobile()) {
        ...
        if (navItem.classList.contains(CLASS_ACTIVE)) {
          var targetCol = targetId ? panel.querySelector("#".concat(CSS.escape(targetId))) : null;
          var parentId = targetCol ? targetCol.getAttribute("data-parent-id") || "filter-level-0" : "filter-level-0";
