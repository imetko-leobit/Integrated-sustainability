...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const floatingBtnsToReveal = [
    ...
    document.querySelector(
      ".floating-filter-btn.floating-filter-btn-request-credentials:not(.floating-filter-panel-btn)"
    ),
  ].filter(Boolean);

  if (floatingBtnsToReveal.length > 0) {
    // Find the trigger section: the .block-section-heading that contains the
    // floating-filter-trigger-btn icon (set via $custom_icon_class in PHP).
      ...
      const revealObserver = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            // Show buttons only when the heading has scrolled above the viewport.
            const hasPassed =
              !entry.isIntersecting && entry.boundingClientRect.top < 0;
            floatingBtnsToReveal.forEach((btn) => {
              btn.classList.toggle("is-scrolled-visible", hasPassed);
            });
          });
        },
        { threshold: 0 }
      );
      revealObserver.observe(triggerSection);
    } else {
      // No trigger section found – show buttons immediately so they are usable.
      floatingBtnsToReveal.forEach((btn) =>
