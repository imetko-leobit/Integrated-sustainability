document.addEventListener("DOMContentLoaded", () => {
  ...
  function goToLevel(targetId, activeItem, isForward) {
    ...
    if (isMobile()) {
      panel.querySelectorAll(".submenu-header--injected").forEach((el) => el.remove());
      const targetCol = panel.querySelector(`#${CSS.escape(targetId)}`);
      if (targetCol) {
        ...
        if (chain.length > 2) {
          ...
          if (existingHeader) {
            for (let i = 1; i <= chain.length - 3; i++) {
              ...
              const labelEl = ancestorCol == null ? void 0 : ancestorCol.querySelector(
                ".submenu-category-name, .main-menu__title"
              );
              const label = ((_a = labelEl == null ? void 0 : labelEl.textContent) == null ? void 0 : _a.trim()) || "";
